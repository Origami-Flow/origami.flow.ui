...
export const CalendarMonth = ({ onClick }) => {
            ...
            <div className="flex w-full" key={`week-${weekIndex}`}>
                {week.map(({ month, day }, dayIndex) => {
                    ...
                    return (
                        ...
                        >
                            <span className={`absolute left-1 top-1 flex size-7 items-center justify-center rounded-full text-lg ${isToday ? 'bg-roseprimary font-semibold text-white' : ''}`}>
                                {day}
                            </span>
                            {isNewMonth && (
            ...
            <div className="sticky -top-px z-50 w-full rounded-t-2xl bg-white px-5 pt-7">
                <div className="mb-4 flex w-full flex-wrap items-center justify-between gap-6">
                    <div className="flex flex-wrap gap-2">
                        ...
                        <button onClick={handleTodayClick} type="button" className="rounded-lg border bg-white px-3 py-1.5 text-sm font-medium">
                            Hoje
                        </button>
                    </div>
                    <div className="flex w-fit items-center justify-between">
                        <button onClick={handlePrevYear} className="rounded-full border p-1">
                            <ChevronLeft/>
                        </button>
                        <h1 className="text-lg font-semibold">{year}</h1>
                        <button onClick={handleNextYear} className="rounded-full border p-1">
                            <ChevronRight/>
                        </button>
                    </div>
                </div>
