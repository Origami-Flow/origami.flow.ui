    ...
    >
      ...
      <div className="flex flex-col h-full py-1 justify-between">
        ...
        <div className="flex flex-col w-full items-center h-1/3 justify-between">
          {book?.previewLink && (
            <Button
              onClick={() => handlePreviewClick(book.previewLink)}
              className="bg-roseprimary text-white w-32 gap-2 font-semibold h-8"
            >
              <BookText />
              Ler Online
            </Button>
          ...
          {book?.downloadLink && (
            <Button onClick={() => handlePreviewClick(book.downloadLink)} className="bg-marromsecundary text-white w-32 gap-3 font-semibold h-8">
              <FileDown />
              Baixar
            </Button>
          )}
